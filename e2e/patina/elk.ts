...
describe(`${app.name} patina (linter)`, () => {
  before(() => {
  ...
  it("vize lint does not crash and snapshot matches", () => {
    const patinaConfig = app.patina!;
    const patterns = patinaConfig.patterns.map((p) => `'${p}'`).join(" ");
    ...
    assertSnapshot(SNAPSHOT_DIR, `${app.name}-patina`, prettyOutput);
  });
});
