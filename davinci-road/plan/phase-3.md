...
*value*; `proven/refuted/unknown` is the epistemic *status* of that value —
a binding can be provenly `reactive` or unknowably classified, and rules fire
only on proven values per the assurance doctrine. *Accept:* declarative rule spec + naive evaluator committed
...
`-O` tiers = budget constants in `budgets.toml`. *Accept:* TS-17 snapshots of decisions; TS-32 remarks record
