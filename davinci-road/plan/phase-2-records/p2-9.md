...
- The in-`src` `#[cfg(any(test, feature))]` comparator shape (P1-7) was
  ...
  the S2 lane runs *beside* the legacy lane. When P2-11 puts a real S2
...
- *Mandatory*: skipping changes meaning — the key fact is what keyed
...
- *Lowering, not Diagnostic*: the pass **mutates** — the key leaves the
...
- *Barrier*: forced by law 1, and independently true — the collision
...
- a `ui.for` branch root keeps its key with the iteration (Vue 3
  precedence);
- **the span-identity guard**: the single root op must *be* the branch
...
- **A measured domain decision**: the first cut skipped on *any* legacy
