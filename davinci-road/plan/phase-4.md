...
unrepresentable (Swift anti-lesson). *Accept:* TS-35; detector self-test.
...
struct-field reads to demands, consumption matrix (TS-12) updated. *Accept:*
...
the decision recorded in the charter. *Accept:* consumption matrix shows no
...
session-vs-CLI dual path — the known incident class). *Accept:* TS-40; TS-25
...
in CI (TS-36). *Accept:* TS-36; a canary rule that tries error-on-unknown
fails to compile.
...
map to input dialects. *Accept:* TS-11 lint surface empty; facade benches
...
wave). *Accept:* TS-39 per wave; TS-12 matrices current; final SFC/JSX
...
rules + Doctor findings; `vize_curator/src/complexity` absorbed. *Accept:*
...
`vize_maestro/src/ide/ecosystem.rs`. *Accept:* route-typing diagnostics on
...
*Accept:* seeded-defect classes for nesting violations at 100% recall
...
deleted); pug via the S1 pug dialect. *Accept:* TS-5 four properties green
...
*Accept:* musea fixture suite (TS-1) green; TS-11 art surface empty.
...
*Accept:* renderer snapshot fixtures (exact, per locale); every error code
...
*Accept:* spike rule runs deterministic + cost-attributed; decision recorded
(GA in P6-7).
