...
| TS-20 | Lowering totality fuzz  | `cargo test -p vize_ricalco` (battery + truncations) + `cargo +nightly fuzz run s1_lowering` / `folio_parse`       | no panic on arbitrary bytes, diagnostics not crashes; id-accounting + folio round-trip laws hold per input       | P2-8  |
| TS-21 | Metamorphic equivalence | `cargo test -p vize_ricalco --test metamorphic` (matrix plane; corpus shard via `VIZE_DAVINCI_METAMORPHIC_CORPUS`) | S2 (later S3) folios identical modulo declared normalization; mutations + skips counted, zero-mutation run fails | P2-15 |
