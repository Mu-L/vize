...
render back exactly — *including malformed input*: broken source becomes
...
*Import:* S1 encodes `Unexpected`/`Missing` as typed node kinds — no per-
...
*Anti-lesson:* don't copy the persistent value-semantic tree implementation
...
*Import:* the capability handshake goes into the WIT world verbatim;
...
binary contract Swift lacked). Compatibility *policy* is written down, not
...
*Import:* per-SFC summaries split into fingerprinted **facets** (signature,
...
runtime.
*Anti-lesson:* queries retrofitted onto mutable state is the negative proof
...
**SIL — mandatory vs optional passes.** SILGen emits *raw* SIL; a fixed set of
**mandatory passes** (definite initialization, coverage diagnostics) runs at
every optimization level and produces *canonical* SIL — the only form later
stages accept; optional optimizations run after
([SIL.md](https://github.com/swiftlang/swift/blob/main/docs/SIL/SIL.md)).
*Import — the most directly adoptable idea in the survey:* every Davinci pass
...
traversal budget.
*Anti-lesson:* OSSA took ~8 years to retrofit — S3's state edges are
...
*Import:* the design property, not the tool — S2/S3 nodes carry enough
...
*Anti-lesson:* type-well-formedness won't capture reactivity semantics the way
...
each module records the fingerprints of everything it *used*; recompilation is
...
invalidation.
*Import:* per-declaration fingerprints + record-what-you-used becomes the
...
*Import:* canonicalization rewrites are closed-world (third parties propose
...
- **Two traversals is enough** — the strongest precedent for the traversal
  ...
  environment.
*Anti-lesson:* the hard fork (link-incompatible variants) is what budget-scaled
...
*Import:* Davinci provenance records carry `(rule name, input node,
...
*Anti-lesson:* InfoTree is memory-heavy — provenance is ring-buffered/off in
...
*Import:* a **third incrementality mode layered under salsa** — salsa at
...
*Import:* stage verifiers take only the serialized stage artifact (the Folio
...
*Import:* each fact group defines (α serialized summary entries, β in-memory
index) with an explicit export function — the α form *is* the per-SFC summary
...
*Anti-lesson:* Lean's global environment means any import's simp lemma affects
...
*Import:* summaries are body-elided **by type construction** (day-one
...
**Round-trip printing.** Lean's `notation` generates parser *and* printer from
...
*Import:* one declarative grammar per Folio stage generating both parser and
