...
| 23 | Compatibility policy (per surface) | **Vize-internal interfaces: not a goal during Davinci** — vize is experimental; crate APIs, CLI internals, dump/cache formats may break freely until contracts GA (phase 6); deprecation aliases are courtesy. **The compiler: compatibility is paramount, tiered by backend** — **VDOM and SSR hold the hard byte-parity bar**: the *gate* is corpus byte-identity of emitted output (behavioral identity with Vue semantics is the *reason* the bar exists, not a second oracle); waivers get the strictest review; **Vapor is an experimental surface** — upstream Vapor is itself fluid, so Vapor holds behavior-level compatibility (observable DOM results, reactivity semantics — sprout-style behavioral oracles) with maximal freedom in emitted shape. **Linter / type checker / semantic analysis: correctness and robustness outrank compatibility** — no bug-for-bug parity chasing with vue-tsc or eslint-plugin-vue; where upstream is wrong, Vize is right and the divergence is documented with its justification. |
