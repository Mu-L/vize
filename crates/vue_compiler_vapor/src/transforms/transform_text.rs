...
use vue_compiler_core::{ExpressionNode, InterpolationNode, SimpleExpressionNode, TextNode};
...
pub fn should_merge_text_nodes(children: &[vue_compiler_core::TemplateChildNode<'_>]) -> bool {
    let mut consecutive_count = 0;
    for child in children {
        match child {
            vue_compiler_core::TemplateChildNode::Text(_)
            | vue_compiler_core::TemplateChildNode::Interpolation(_) => {
                consecutive_count += 1;
