...
use vize_s0::{Allocator, CompactString};
use vize_s1::{SurfaceChild, SurfaceTree, check_fidelity, parse, parse_with_authored};
...
fn ordinary_and_foreign_namespace_sources_reuse_the_existing_surface() {
    for source in [
        ...
        assert_eq!(
            vize_s0::cstr!("{normal:?}"),
            vize_s0::cstr!("{reference:?}")
        );
        assert_eq!(
            vize_s0::cstr!("{errors:?}"),
            vize_s0::cstr!("{reference_errors:?}")
        );
    }
}
