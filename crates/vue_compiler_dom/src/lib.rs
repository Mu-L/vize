...
use vue_compiler_core::codegen::CodegenResult;
...
pub fn compile_template_with_options<'a>(
    ...
    let parser_opts = ParserOptions {
        is_void_tag: vue_shared::is_void_tag,
        is_native_tag: Some(vue_shared::is_native_tag),
    ...
    if !errors.is_empty() {
        let codegen_result = CodegenResult {
            code: vue_allocator::String::default(),
            preamble: vue_allocator::String::default(),
            map: None,
        };
    ...
    let binding_metadata = options.binding_metadata.as_ref().map(|map| {
        use vue_compiler_core::options::{BindingMetadata, BindingType};
        ...
        for (name, type_str) in &map.bindings {
            ...
            bindings.insert(vue_allocator::String::from(name.as_str()), binding_type);
        }
...
fn get_namespace(tag: &str, parent: Option<&str>) -> Namespace {
    if vue_shared::is_svg_tag(tag) {
        return Namespace::Svg;
    }
    if vue_shared::is_math_ml_tag(tag) {
        return Namespace::MathMl;
    }

    // Inherit namespace from parent
    if let Some(parent_tag) = parent {
        if vue_shared::is_svg_tag(parent_tag) && tag != "foreignObject" {
            return Namespace::Svg;
        }
        if vue_shared::is_math_ml_tag(parent_tag)
            && tag != "annotation-xml"
            && tag != "foreignObject"
        {
            return Namespace::MathMl;
