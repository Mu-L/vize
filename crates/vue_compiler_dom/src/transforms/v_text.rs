...
use vue_compiler_core::{DirectiveNode, RuntimeHelper};
...
pub fn generate_text_content(dir: &DirectiveNode<'_>) -> String {
    if let Some(ref exp) = dir.exp {
        if let vue_compiler_core::ExpressionNode::Simple(simple) = exp {
...
#[cfg(test)]
mod tests {
    use vue_allocator::{Box, Bump};
    use vue_compiler_core::{DirectiveNode, ExpressionNode, SimpleExpressionNode, SourceLocation};
