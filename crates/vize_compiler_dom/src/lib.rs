...
use vize_compiler_core::codegen::CodegenResult;
...
pub fn compile_template_with_options<'a>(
    ...
    let binding_metadata = options.binding_metadata.as_ref().map(|map| {
        use vize_compiler_core::options::{BindingMetadata, BindingType};
        ...
        for (name, type_str) in &map.bindings {
            ...
            bindings.insert(vize_allocator::String::from(name.as_str()), binding_type);
        }
...
fn get_namespace(tag: &str, parent: Option<&str>) -> Namespace {
    if vize_shared::is_svg_tag(tag) {
        return Namespace::Svg;
    }
    if vize_shared::is_math_ml_tag(tag) {
        return Namespace::MathMl;
    }

    // Inherit namespace from parent
    if let Some(parent_tag) = parent {
        if vize_shared::is_svg_tag(parent_tag) && tag != "foreignObject" {
            return Namespace::Svg;
        }
        if vize_shared::is_math_ml_tag(parent_tag)
            && tag != "annotation-xml"
            && tag != "foreignObject"
        {
            return Namespace::MathMl;
...
mod tests {
    use super::*;
    use vize_compiler_core::options::CodegenMode;
