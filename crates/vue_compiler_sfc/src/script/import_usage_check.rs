...
use vue_compiler_core::ast::{
    DirectiveNode, ElementNode, ExpressionNode, PropNode, RootNode, SimpleExpressionNode,
    TemplateChildNode,
};
use vue_shared::{camelize, capitalize, is_builtin_directive, is_native_tag, is_simple_identifier};
        ...
        TemplateChildNode::TextCall(text_call) => {
            if collect_used_ids {
                match &text_call.content {
                    vue_compiler_core::ast::TextCallContent::Interpolation(interp) => {
                        extract_identifiers_from_expression(&interp.content, &mut result.used_ids);
                    }
                    vue_compiler_core::ast::TextCallContent::Compound(compound) => {
...
fn extract_identifiers_from_compound(
    node: &vue_compiler_core::ast::CompoundExpressionNode,
    ids: &mut FxHashSet<String>,
) {
    ...
    for child in node.children.iter() {
        match child {
            vue_compiler_core::ast::CompoundExpressionChild::Simple(simple) => {
                extract_identifiers_from_simple_expression(simple, ids);
            }
            vue_compiler_core::ast::CompoundExpressionChild::Compound(compound) => {
                extract_identifiers_from_compound(compound, ids);
            }
            vue_compiler_core::ast::CompoundExpressionChild::Interpolation(interp) => {
