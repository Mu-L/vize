...
| 23 | Vize-internal compatibility | **Not a goal during Davinci** — vize is experimental (Real World Testing). Crate APIs, CLI internals, dump and cache formats may break freely until contracts GA (phase 6); deprecation aliases are courtesy, not obligation. What must hold instead: **Vue-semantics parity** (the corpus gates are regression oracles, not compatibility promises — intentionally different-but-correct output is waiver-reviewed, not forbidden) and the working experience of Real World Testing users. |
