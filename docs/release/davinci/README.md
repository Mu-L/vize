...
| 22 | Priority order | **Performance and correctness are co-equal and supreme. Size and extensibility come second. Implementation simplicity is explicitly not a goal** — difficult is fine. "Performance" includes memory (peak and steady-state) as one budget family. Trade-off resolution: performance/correctness never yield; size holds its budgets (#19) against performance's appetite (monomorphization bloat is watched, not banned); extensibility never taxes the hot path (#15's two tiers exist for this); and a simpler implementation loses to all of the above. Complexity is unbounded *inside the disciplines* — contracts, verifiers, Folio inspectability — which is how this coexists with `ubugeeei-redundancy.md`'s "do not accumulate hacks as architecture": hard code with a verifier and a dump is engineering; hard code without them is a hack. |
| 23 | Compatibility policy (per surface) | **Vize-internal interfaces: not a goal during Davinci** — vize is experimental; crate APIs, CLI internals, dump/cache formats may break freely until contracts GA (phase 6); deprecation aliases are courtesy. **The compiler: compatibility is paramount, tiered by backend** — **VDOM and SSR hold the hard byte-parity bar** (emitted code behaves identically to Vue semantics; real apps depend on it; waivers get the strictest review); **Vapor is an experimental surface** — upstream Vapor is itself fluid, so Vapor holds behavior-level compatibility (observable DOM results, reactivity semantics — sprout-style behavioral oracles) with maximal freedom in emitted shape. **Linter / type checker / semantic analysis: correctness and robustness outrank compatibility** — no bug-for-bug parity chasing with vue-tsc or eslint-plugin-vue; where upstream is wrong, Vize is right and the divergence is documented with its justification. |
...
| 29 | JS plugin tier | **Users write plugins and custom rules in JavaScript** — a third extension tier alongside compiled-in Rust and WASM dialects. JS rules target the same neutral-core S2 view and declare fact demands like Rust rules, but run **outside the fused walks** in a batched pass over the napi/vitrine bridge, with per-plugin cost attribution (a slow rule is visible, never hidden) and content-keyed caching so JS rules are incremental like everything else. The hot path never waits on user JS. |
