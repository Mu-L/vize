...
| 23 | Compatibility policy (per surface) | **Vize-internal interfaces: not a goal during Davinci** — vize is experimental; crate APIs, CLI internals, dump/cache formats may break freely until contracts GA (phase 6); deprecation aliases are courtesy. **The compiler: compatibility is paramount** — emitted code must behave identically to Vue's semantics; real apps depend on it; corpus byte-parity is the hard bar for compile surfaces and compile waivers get the strictest review. **Linter / type checker / semantic analysis: correctness and robustness outrank compatibility** — no bug-for-bug parity chasing with vue-tsc or eslint-plugin-vue; where upstream is wrong, Vize is right and the divergence is documented with its justification (the existing language-engineering-practices divergence rule). |
