...
- A bounded `v-once` plain element/text subtree projects the S3 directive into
  mount-only shared IR operations. TS-33 compares frozen prop/text in both a
  direct child and a nested `<span>`, plus a still-live sibling, against official
  compiler-vapor through the same published runtime, including node identity.
  Before this slice, the nested source selected `legacy.operation` and the
  retained lane changed its title/text from `first`/`A` to `second`/`B`; official
  Vapor kept `first`/`A`. Events, components, other directives and control-flow
  beneath `v-once` still select `legacy.operation`; official Vapor freezes a
  `v-if v-once` branch's condition, which needs a separate S3 control-flow
  contract.
