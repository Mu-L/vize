...
//!
//! Usage:
//!   cargo run -p vue_test_runner --bin coverage          # Summary only
//!   cargo run -p vue_test_runner --bin coverage -- -v    # Show failing tests
//!   cargo run -p vue_test_runner --bin coverage -- -vv   # Show diffs

use std::path::PathBuf;
use vue_test_runner::{run_fixture_tests, CompilerMode};
